	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class OpportunitynoteImpl {
		 protected static Log logger = LogFactory.getLog(OpportunitynoteImpl.class); 
		 ...
		 public String OpportunitynoteFilter="";
		 ...
		 public String getParentobjid(){
		  ...
		  public  TemplateTable  DogetPostSelect(String[] column,String[] datatype,String parentfilter,boolean isform){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Opportunitynote",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Opportunitynote",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  DogetPostSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Opportunitynote",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Opportunitynote",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   DogetPostDelete(String[] childtabs){
			...
			String sql=query.removeSql("Opportunitynote",this.getParentobjid(),childtabs);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Opportunitynote",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Opportunitynote",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Opportunitynote",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Opportunitynote",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  DogetPostInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					...
					tu.applyObjectRule("Opportunitynote",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Opportunitynote",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
			...
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				 ...
				 tu.applyObjectRule("Opportunitynote",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Opportunitynote","Opportunitynote",getParentobjid());
				 EventListener.registerPostInsertEvent("Opportunitynote",maindata);
				 return(true);
			}
			 return(false);
		}
