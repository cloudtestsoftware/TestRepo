		...
		import com.testrepo.dao.TimesheetDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class TimesheetService {
			...
			public Items getTimesheetForm() {
				Items items = null;
				try {
					items=new TimesheetDao(uriInfo,header).getTimesheetForm();
				} catch (AuthenticationException e) {
			...
			// Get Timesheet record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getTimesheetUpdate() {
				Rows rows = null;
				try {
					rows=new TimesheetDao(uriInfo,header).getTimesheetRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getTimesheetUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
