	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class BugImpl {
		 protected static Log logger = LogFactory.getLog(BugImpl.class); 
		 ...
		 public String getParentobjid(){
		  ...
		  public  TemplateTable  DogetPostSelect(String[] column,String[] datatype,String parentfilter,boolean isform){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Bug",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Bug",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  DogetPostSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Bug",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Bug",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   DogetPostDelete(String[] childtabs){
			...
			String sql=query.removeSql("Bug",this.getParentobjid(),childtabs);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Bug",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Bug",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Bug",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Bug",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  DogetPostInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					...
					tu.applyObjectRule("Bug",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Bug",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
			...
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				 ...
				 tu.applyObjectRule("Bug",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Bug","Bug",getParentobjid());
				 EventListener.registerPostInsertEvent("Bug",maindata);
				 return(true);
			}
			 return(false);
		}
