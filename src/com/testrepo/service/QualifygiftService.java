		...
		import com.testrepo.dao.QualifygiftDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class QualifygiftService {
			...
			// Get Qualifygift record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getQualifygiftUpdate() {
				Rows rows = null;
				try {
					rows=new QualifygiftDao(uriInfo,header).getQualifygiftRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getQualifygiftUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
