		...
		import com.testrepo.dao.ModuleDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class ModuleService {
			...
			public Items getModuleForm() {
				Items items = null;
				try {
					items=new ModuleDao(uriInfo,header).getModuleForm();
				} catch (AuthenticationException e) {
			...
			// Get Module record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getModuleUpdate() {
				Rows rows = null;
				try {
					rows=new ModuleDao(uriInfo,header).getModuleRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getModuleUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
