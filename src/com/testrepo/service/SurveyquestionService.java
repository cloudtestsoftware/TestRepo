		...
		import com.testrepo.dao.SurveyquestionDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class SurveyquestionService {
			...
			// Get Surveyquestion record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getSurveyquestionUpdate() {
				Rows rows = null;
				try {
					rows=new SurveyquestionDao(uriInfo,header).getSurveyquestionRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getSurveyquestionUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
