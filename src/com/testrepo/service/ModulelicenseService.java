		...
		import com.testrepo.dao.ModulelicenseDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class ModulelicenseService {
			...
			public Items getModulelicenseForm() {
				Items items = null;
				try {
					items=new ModulelicenseDao(uriInfo,header).getModulelicenseForm();
				} catch (AuthenticationException e) {
			...
			// Get Modulelicense record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getModulelicenseUpdate() {
				Rows rows = null;
				try {
					rows=new ModulelicenseDao(uriInfo,header).getModulelicenseRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getModulelicenseUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
