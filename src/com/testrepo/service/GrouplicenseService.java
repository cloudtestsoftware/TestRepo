		...
		import com.testrepo.dao.GrouplicenseDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class GrouplicenseService {
			...
			public Items getGrouplicenseForm() {
				Items items = null;
				try {
					items=new GrouplicenseDao(uriInfo,header).getGrouplicenseForm();
				} catch (AuthenticationException e) {
			...
			// Get Grouplicense record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getGrouplicenseUpdate() {
				Rows rows = null;
				try {
					rows=new GrouplicenseDao(uriInfo,header).getGrouplicenseRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getGrouplicenseUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
