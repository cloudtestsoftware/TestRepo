		...
		import com.testrepo.dao.SubscriptionDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class SubscriptionService {
			...
			public Items getSubscriptionForm() {
				Items items = null;
				try {
					items=new SubscriptionDao(uriInfo,header).getSubscriptionForm();
				} catch (AuthenticationException e) {
			...
			// Get Subscription record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getSubscriptionUpdate() {
				Rows rows = null;
				try {
					rows=new SubscriptionDao(uriInfo,header).getSubscriptionRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getSubscriptionUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
