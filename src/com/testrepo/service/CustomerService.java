		...
		import com.testrepo.dao.CustomerDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class CustomerService {
			...
			// Get Customer record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getCustomerUpdate() {
				Rows rows = null;
				try {
					rows=new CustomerDao(uriInfo,header).getCustomerRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getCustomerUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
