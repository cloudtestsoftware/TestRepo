		...
		import com.testrepo.dao.ContactnoteDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class ContactnoteService {
			...
			// Get Contactnote record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getContactnoteUpdate() {
				Rows rows = null;
				try {
					rows=new ContactnoteDao(uriInfo,header).getContactnoteRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getContactnoteUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
