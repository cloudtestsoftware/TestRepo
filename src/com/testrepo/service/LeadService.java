		...
		import com.testrepo.dao.LeadDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class LeadService {
			...
			// Get Lead record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getLeadUpdate() {
				Rows rows = null;
				try {
					rows=new LeadDao(uriInfo,header).getLeadRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getLeadUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
