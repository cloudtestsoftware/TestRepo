		...
		import com.testrepo.dao.OpportunitynoteDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class OpportunitynoteService {
			...
			// Get Opportunitynote record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getOpportunitynoteUpdate() {
				Rows rows = null;
				try {
					rows=new OpportunitynoteDao(uriInfo,header).getOpportunitynoteRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getOpportunitynoteUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
