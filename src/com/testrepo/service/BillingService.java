		...
		import com.testrepo.dao.BillingDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class BillingService {
			...
			public Items getBillingForm() {
				Items items = null;
				try {
					items=new BillingDao(uriInfo,header).getBillingForm();
				} catch (AuthenticationException e) {
			...
			// Get Billing record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getBillingUpdate() {
				Rows rows = null;
				try {
					rows=new BillingDao(uriInfo,header).getBillingRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getBillingUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
