		...
		import com.testrepo.dao.MonthDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class MonthService {
			...
			public Items getMonthForm() {
				Items items = null;
				try {
					items=new MonthDao(uriInfo,header).getMonthForm();
				} catch (AuthenticationException e) {
			...
			// Get Month record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getMonthUpdate() {
				Rows rows = null;
				try {
					rows=new MonthDao(uriInfo,header).getMonthRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getMonthUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
