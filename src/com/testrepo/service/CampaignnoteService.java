		...
		import com.testrepo.dao.CampaignnoteDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class CampaignnoteService {
			...
			// Get Campaignnote record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getCampaignnoteUpdate() {
				Rows rows = null;
				try {
					rows=new CampaignnoteDao(uriInfo,header).getCampaignnoteRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getCampaignnoteUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
