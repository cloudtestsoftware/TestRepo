		...
		import com.testrepo.dao.OpportunityDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class OpportunityService {
			...
			// Get Opportunity record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getOpportunityUpdate() {
				Rows rows = null;
				try {
					rows=new OpportunityDao(uriInfo,header).getOpportunityRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getOpportunityUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
