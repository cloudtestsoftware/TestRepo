		...
		import com.testrepo.dao.PrivilegegroupDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class PrivilegegroupService {
			...
			public Items getPrivilegegroupForm() {
				Items items = null;
				try {
					items=new PrivilegegroupDao(uriInfo,header).getPrivilegegroupForm();
				} catch (AuthenticationException e) {
			...
			// Get Privilegegroup record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getPrivilegegroupUpdate() {
				Rows rows = null;
				try {
					rows=new PrivilegegroupDao(uriInfo,header).getPrivilegegroupRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getPrivilegegroupUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
			...
			// Get all Testuser rows against object ID for Privilegegroup
			@GET
			@Path("/{id}/testuser")
			@Produces({"application/xml"})
			public Rows getTestuserRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new PrivilegegroupDao(uriInfo,header).getTestuserRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
