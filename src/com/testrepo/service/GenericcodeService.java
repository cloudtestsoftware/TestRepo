		...
		import com.testrepo.dao.GenericcodeDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class GenericcodeService {
			...
			public Items getGenericcodeForm() {
				Items items = null;
				try {
					items=new GenericcodeDao(uriInfo,header).getGenericcodeForm();
				} catch (AuthenticationException e) {
			...
			// Get Genericcode record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getGenericcodeUpdate() {
				Rows rows = null;
				try {
					rows=new GenericcodeDao(uriInfo,header).getGenericcodeRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getGenericcodeUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
