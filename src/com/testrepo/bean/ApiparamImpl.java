	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class ApiparamImpl {
		 protected static Log logger = LogFactory.getLog(ApiparamImpl.class); 
		 ...
		 public String getParentobjid(){
		  ...
		  public  TemplateTable  DogetPostSelect(String[] column,String[] datatype,String parentfilter,boolean isform){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Apiparam",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Apiparam",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  DogetPostSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Apiparam",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Apiparam",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   DogetPostDelete(String[] childtabs){
			...
			String sql=query.removeSql("Apiparam",this.getParentobjid(),childtabs);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Apiparam",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Apiparam",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Apiparam",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Apiparam",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  DogetPostInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					...
					tu.applyObjectRule("Apiparam",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Apiparam",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
			...
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				 ...
				 tu.applyObjectRule("Apiparam",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Apiparam","Apiparam",getParentobjid());
				 EventListener.registerPostInsertEvent("Apiparam",maindata);
				 return(true);
			}
			 return(false);
		}
