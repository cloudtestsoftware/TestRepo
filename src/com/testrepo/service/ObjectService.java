		...
		import com.testrepo.dao.ObjectDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class ObjectService {
			...
			public Items getObjectForm() {
				Items items = null;
				try {
					items=new ObjectDao(uriInfo,header).getObjectForm();
				} catch (AuthenticationException e) {
			...
			// Get Object record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getObjectUpdate() {
				Rows rows = null;
				try {
					rows=new ObjectDao(uriInfo,header).getObjectRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getObjectUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
