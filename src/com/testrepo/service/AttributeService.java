		...
		import com.testrepo.dao.AttributeDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class AttributeService {
			...
			public Items getAttributeForm() {
				Items items = null;
				try {
					items=new AttributeDao(uriInfo,header).getAttributeForm();
				} catch (AuthenticationException e) {
			...
			// Get Attribute record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getAttributeUpdate() {
				Rows rows = null;
				try {
					rows=new AttributeDao(uriInfo,header).getAttributeRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getAttributeUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
