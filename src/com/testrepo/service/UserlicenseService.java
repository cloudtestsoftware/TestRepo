		...
		import com.testrepo.dao.UserlicenseDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class UserlicenseService {
			...
			public Items getUserlicenseForm() {
				Items items = null;
				try {
					items=new UserlicenseDao(uriInfo,header).getUserlicenseForm();
				} catch (AuthenticationException e) {
			...
			// Get Userlicense record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getUserlicenseUpdate() {
				Rows rows = null;
				try {
					rows=new UserlicenseDao(uriInfo,header).getUserlicenseRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getUserlicenseUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
