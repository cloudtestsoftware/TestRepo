		...
		import com.testrepo.dao.ObjectruleDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class ObjectruleService {
			...
			public Items getObjectruleForm() {
				Items items = null;
				try {
					items=new ObjectruleDao(uriInfo,header).getObjectruleForm();
				} catch (AuthenticationException e) {
			...
			// Get Objectrule record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getObjectruleUpdate() {
				Rows rows = null;
				try {
					rows=new ObjectruleDao(uriInfo,header).getObjectruleRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getObjectruleUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
