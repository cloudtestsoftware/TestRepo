		...
		import com.testrepo.dao.EmailsettingDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class EmailsettingService {
			...
			// Get Emailsetting record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getEmailsettingUpdate() {
				Rows rows = null;
				try {
					rows=new EmailsettingDao(uriInfo,header).getEmailsettingRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getEmailsettingUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
