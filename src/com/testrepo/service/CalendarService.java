		...
		import com.testrepo.dao.CalendarDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class CalendarService {
			...
			public Items getCalendarForm() {
				Items items = null;
				try {
					items=new CalendarDao(uriInfo,header).getCalendarForm();
				} catch (AuthenticationException e) {
			...
			// Get Calendar record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getCalendarUpdate() {
				Rows rows = null;
				try {
					rows=new CalendarDao(uriInfo,header).getCalendarRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getCalendarUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
