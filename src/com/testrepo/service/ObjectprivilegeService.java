		...
		import com.testrepo.dao.ObjectprivilegeDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class ObjectprivilegeService {
			...
			// Get Objectprivilege record update by id
			@GET
			@Path("/{id}/update")
			@Produces({"application/xml"})
			public Rows getObjectprivilegeUpdate() {
				Rows rows = null;
				try {
					rows=new ObjectprivilegeDao(uriInfo,header).getObjectprivilegeRowUpdated();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
					 logger.info( "Error calling getObjectprivilegeUpdate()"+ ex.getMessage());
					 ex.printStackTrace();
				}
				return rows;
			}
